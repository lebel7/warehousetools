...
import com.proper.data.goodsin.GoodsInImagePath;
import com.proper.data.goodsin.GoodsInImagesRequest;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if (extras == null) {
            ...
            if (boardScanResult == null) {
                throw new RuntimeException("ActGoodsInReceive - ScanResult Extras cannot be null");
            }
        }
        ...
        txtSupplier.setText(boardScanResult.getGoodsIn().getSupplierName());
    ...
    private void buttonClicked(View v) {
        ...
        if (v == btnSend) {
            ...
            if (mPhotoPathUri != null && !mPhotoPathUri.toString().isEmpty() && pictureCount > 0) {
                ...
                img = new GoodsInImage(boardScanResult.getGoodsIn().getSupplierName(), boardScanResult.getDelivery().getGoodsInId(), 0, boardScanResult.getGoodsIn().getOrderNumber(), boardScanResult.getGoodsIn().getUnitsOrdered(), userDetail, currentGallery);
        ...
        if (v == imgPhoto) {
            if (mPhotoPathUri != null) {
                Intent i = new Intent(ActGoodsInReceive.this, ActGoodsInReceive.class);
        ...
        protected AbstractMap.SimpleEntry<Boolean, AbstractMap.SimpleEntry<String, String>> doInBackground(GoodsInImage... images) {
            ...
            if (resolved.getKey() == true) {
                try {
                    ...
                    List<GoodsInImagePath> paths = new ArrayList<GoodsInImagePath>();
                    for (int i = 0; i < currentGallery.size(); i++) {
                        File file  = new File(currentGallery.get(i).getPath());
                        paths.add(new GoodsInImagePath(FilenameUtils.concat(resolved.getValue(), file.getName())));
                    }
                    ...
                    //Finally Send an automated email
            ...
            if (response != null) {
                if (response.getKey() == true) {
                    if (response.getValue().getKey().isEmpty()) {
                        ...
                        String mMsg = "Files uploaded successfully but message service failed.\nPlease contact IT if error continues";
                        builder.setMessage(mMsg)
                                .setPositiveButton(R.string.but_ok, new DialogInterface.OnClickListener() {
                                    public void onClick(DialogInterface dialog, int id) {
                                        //do nothing
                        ...
                        i.putExtra("PHOTOLOCATION_EXTRA", response.getValue());
